...
from imdb_model import get_embedding, grad_x_input_imdb
...
gxi = True
if gxi:
    X_tr_orig = grad_x_input_imdb(model, X_tr_orig)
    X_val_orig = grad_x_input_imdb(model, X_val_orig)
...
for shift_idx, shift in enumerate(shifts):
    ...
    for rand_run in range(random_runs):
        ...
        # Apply shift.
        if shift is not 'orig':
            (_, _), (_, _), (X_te_1, _), _, _ = import_dataset('imdb_perturbed')

        if gxi:
            print("Generating gradxinput")
            # convert to explanations
            X_te_1 = grad_x_input_imdb(model, X_te_1)
        ...
        for si, sample in enumerate(samples):
            ...
            if test_type == 'multiv':
                ...
                model, score, (X_tr_dcl, y_tr_dcl, y_tr_old, X_te_dcl, y_te_dcl, y_te_old) = shift_locator.build_model(
                    X_tr_3, y_tr_3, X_te_3, y_te_3)
        ...
        for dr_idx, dr in enumerate(dr_techniques_plot):
            plt.semilogx(np.array(samples), rand_run_p_vals[:, dr_idx, rand_run], format[dr], color=colors[dr],
...
for dr_idx, dr in enumerate(dr_techniques_plot):
    ...
    for idx, shift in enumerate(shifts):
        errorfill(np.array(samples), mean_p_vals[:, idx], std_p_vals[:, idx], fmt=linestyles[idx] + markers[dr],
                  color=colorscale(colors[dr], brightness[idx]), label="%s" % shift.replace('_', '\\_'))
    plt.xlabel('Number of samples from test')
    plt.ylabel('$p$-value')
    plt.axhline(y=sign_level, color='k')
